...
class radmc3dModel(object):
    ...
    def readParams(self, fdir=None):

        self.par = analyze.readParams(fdir=fdir)
    ...
    def makeVar(self, ddens=False, dtemp=False, gdens=False, gtemp=False, gvel=False, vturb=False, extern_source=False, alvec=False, qvis=False,
                     writeToFile=False, fdir=None):
        """
        ...
        ----------
        ddens           : bool, optional
                          If True generates the dust density. needs getDustDensity()

        dtemp           : bool, optional
                          If True generates the dust temperature (normally this would be calculated by RADMC-3D, but
                            in some cases, e.g. debugging, it might come handy). getDustTemperature()

        gdens           : bool, optional
                          If True generates the gas density. getGasDensity(), getGasAbundance()

        gtemp           : bool, optional
                          If True generates the gas temperature. getGasTemperature()

        gvel            : bool, optional
                          If True generates the gas velocity. getVelocity()

        vturb           : bool, optional
                          If True generates the microturbulent velocity field. getVturb()

        alvec           : bool, optional
                          if True, generates the dust alignment vector field. getDustAlignment()

        qvis            : bool, optional
                          If True, generates the heating for heatsource.inp. needs getViscousHeating() function

        writeToFile     : bool, optional
                          If True the generated variable(s) will be written to file
        fdir            : string, optional
                          The name of directory to write the file to 
        ...
        if ddens:
            ...
            if writeToFile:
                if self.par.ppar['grid_style'] == 1:
                    self.data.writeDustDens(binary=self.binary, octree=True, fdir=fdir)
                else:
                    self.data.writeDustDens(binary=self.binary, old=self.old, fdir=fdir)
        # --------------------------------------------------------------------------------------------
        ...
        if dtemp:
            ...
            if writeToFile:
                if self.par.ppar['grid_style'] == 1:
                    self.data.writeDustTemp(binary=self.binary, octree=True, fdir=fdir)
                else:
                    self.data.writeDustTemp(binary=self.binary, fdir=fdir)
                ...
                if self.par.ppar['grid_style'] == 1:
                    for imol in range(len(self.par.ppar['gasspec_mol_name'])):
                        self.data.writeGasDens(ispec=self.par.ppar['gasspec_mol_name'][imol], binary=self.binary,
                                               octree=True, fdir=fdir)

                    if abs(self.par.ppar['lines_mode']) > 2:
                        for icp in range(len(self.par.ppar['gasspec_colpart_name'])):
                            self.data.writeGasDens(ispec=self.par.ppar['gasspec_colpart_name'][icp],
                                                   binary=self.binary, octree=True, fdir=fdir)
                else:
                    for imol in range(len(self.par.ppar['gasspec_mol_name'])):
                        self.data.writeGasDens(ispec=self.par.ppar['gasspec_mol_name'][imol], binary=self.binary, fdir=fdir)

                    if abs(self.par.ppar['lines_mode']) > 2:
                        for icp in range(len(self.par.ppar['gasspec_colpart_name'])):
                            self.data.writeGasDens(ispec=self.par.ppar['gasspec_colpart_name'][icp], binary=self.binary, fdir=fdir)
        ...
        if gvel:
            ...
            if writeToFile:
                ...
                if self.par.ppar['grid_style'] == 1:
                    self.data.writeGasVel(binary=self.binary, octree=True, fdir=fdir)
                else:
                    self.data.writeGasVel(binary=self.binary, fdir=fdir)
        # --------------------------------------------------------------------------------------------
        ...
        if gtemp:
            ...
            if writeToFile:
                ...
                if self.par.ppar['grid_style'] == 1:
                    self.data.writeGasTemp(binary=self.binary, octree=True, fdir=fdir)
                else:
                    self.data.writeGasTemp(binary=self.binary, fdir=fdir)
        # --------------------------------------------------------------------------------------------
        ...
        if vturb:
            ...
            if writeToFile:
                if self.par.ppar['grid_style'] == 1:
                    self.data.writeVTurb(binary=self.binary, octree=True, fdir=fdir)
                else:
                    self.data.writeVTurb(binary=self.binary, fdir=fdir)

        # --------------------------------------------------------------------------------------------
        # Get the dust alignment vector field
        # --------------------------------------------------------------------------------------------
        if alvec:
            if dir(mdl).__contains__('getDustAlignment'):
                if callable(getattr(mdl, 'getDustAlignment')):
                    self.data.alvec = mdl.getDustAlignment(grid=self.grid, ppar=self.par.ppar)
            else:
                raise RuntimeError(' ' + self.model + '.py does not contain a getDustAlignment() function, therefore, '
                                   + ' grainalign_dir.inp cannot be written')


            if writeToFile:
                if self.par.ppar['grid_style'] == 1:
                    self.data.writeDustAlign(binary=self.binary, octree=True, fdir=fdir)
                else:
                    self.data.writeDustAlign(binary=self.binary, fdir=fdir)

        # --------------------------------------------------------------------------------------------
        # Get the viscous heating
        # --------------------------------------------------------------------------------------------
        if qvis:
            if dir(mdl).__contains__('getViscousHeating'):
                if callable(getattr(mdl, 'getViscousHeating')):
                    self.data.qvis = mdl.getViscousHeating(grid=self.grid, ppar=self.par.ppar)
            else:
                raise RuntimeError(' ' + self.model + '.py does not contain a getViscousHeating() function, therefore, '
                                   + ' heatsource.inp cannot be written')


            if writeToFile:
                if self.par.ppar['grid_style'] == 1:
                    self.data.writeViscousHeating(binary=self.binary, octree=True, fdir=fdir)
                else:
                    self.data.writeViscousHeating(binary=self.binary, fdir=fdir)

    def makeGrid(self, sgrid=True, wgrid=True, writeToFile=False, fdir=None, **kwargs):
        """
        ...
        **kwargs        : Any varible name in problem_params.inp can be used as a keyword argument.
        ...
        if writeToFile:
            ...
            if self.par.ppar['grid_style'] == 1:
                # Frequency grid
                self.grid.writeWavelengthGrid(old=False, fdir=fdir)
                # Spatial grid
                self.grid.writeSpatialGrid(old=False, fdir=fdir)
            else:
                # Frequency grid
                self.grid.writeWavelengthGrid(old=self.old, fdir=fdir)
                # Spatial grid
                self.grid.writeSpatialGrid(old=self.old, fdir=fdir)

    def makeRadSources(self, writeToFile=True, fdir=None):
        """
        ...
        writeToFile     : bool
                          ...
                          stellarsrc_templates.inp, external_source.inp
        ...
        # get the External source irradiation
        self.radsources.getExternalSource(grid=self.grid, ppar=self.par.ppar)

        if writeToFile:
            # Input radiation field (discrete stars)
            self.radsources.writeStarsinp(ppar=self.par.ppar, old=self.old, wav=self.grid.wav, fdir=fdir)
            ...
            if not self.old:
                ...
                totlum = self.radsources.getTotalLuminosities(readInput=True, fdir=fdir)
                print('As calculated from the input files :')
                print('Stars : ')
                print("  Star #%d + hotspot        : %.6e (Lsun)" % (0, totlum['lnu_star'][0]/nc.ls))
            ...
            self.radsources.writeExternalSource(fdir=fdir)
    ...
    def writeRadmc3dInp(self, fdir=None):
        """
        Writes the radmc3d.inp master command file for RADMC-3D
        """
        writeRadmc3dInp(self.par, fdir=fdir)
    ...
    def setupDust(self, sgrid=True, wgrid=True, radsources=True, dustopac=True, ddens=True, dtemp=False, alvec=False):
        """
        ...
        if alvec:
            self.makeVar(alvec=True, writeToFile=True)
...
def writeRadmc3dInp(modpar=None, fdir=None):
    """Writes the radmc3d.inp master command file for RADMC-3D
    ...
    fdir     : string, optional
               name of directory to write this file
    ...
    if fdir is None:
        ...
        if fdir[-1] is '/':
            fname = fdir + 'radmc3d.inp'
        else:
            fname = fdir + '/radmc3d.inp'
    print('Writing '+fname)

    # parameter names that can be float
    # update the list as you code along
    floatkey = ['temp0', 'temp1'] 
