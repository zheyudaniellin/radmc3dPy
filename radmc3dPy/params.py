...
- 'verbose' parameter seems incorrect >> 'miescat_verbose' is what is used
- 'incl_lines' parameter to whether or not include lines. belongs to "Code Parameters"
- 'incl_dust' paramter to whether or not include dust in imaging. belongs to "Code Parameters"
...
class radmc3dPar(object):
    ...
    def readPar(self, fdir=None):
        ...
        fdir  : str, optional
                directory to find problem_params.inp
    ...
    def loadDefaults(self, model='', ppar=None, reset=True):
        ...
        self.setPar(
            ['incl_cont_stellarsrc', 'False', '# Switches on (True) or off (False) continuous stellar sources )',
             'Radiation sources'])
        self.setPar(['ext_source', '0', ' temperature of external irradiation source. less than 0.01 will not write anything',
             'Radiation sources'])
        #
        # Grid parameters
        #
...
#                                         'is doing if it becomes slow.', 'Dust opacity'])
        self.setPar(['miescat_verbose', 'False', ' Give feedback to user.', 'Dust opacity'])
        ...
        self.setPar(['mixgsize', '0', ' 0-no size mixing, 1-to mix grain sizes', 'Dust opacity'])
        ...
        self.setPar(['incl_lines', '1', 
            ' Switch on to include lines for imaging. else (0) to exclude lines', 'Code parameters'])
        self.setPar(['incl_dust', '1',
            ' Switch on to include dust for imaging. else (0) to exclude lines', 'Code parameters'])
        self.setPar(['mc_scat_maxtauabs', '30', ' level of absorption optical depth before photon is extinct.', 
            'Code parameters'])
        self.setPar(['setthreads', '1', '  number of processors', 'Code parameters'])
        self.setPar(['alignment_mode', '0', '  (0) for no alignment, (1) for alignment, (-1) for alignment in last scatterin??', 'Code parameters'])
        self.setPar(['temp0', '0.01', ' lowest pre-calculated temperature', 'Code parameters'])
        self.setPar(['temp1', '1e5', ' highest pre-calculated temperature', 'Code parameters'])
        #
        # Model parameters
        #
        if model != '':
