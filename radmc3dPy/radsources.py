...
fixes/options I did:
- include attributes to set up external_source.inp: getExternalSource, readExternalSource,
    writeExternalSource
"""
...
class radmc3dRadSources(object):
    ...
    ext_inten     : ndarray
                    Intensity of external radiation source. must be same size as the wavelength_micron.inp
    ...
    def __init__(self, ppar=None, grid=None):
        ...
        # External irradiation intensity
        self.ext_inten = []
    ...
    def readStarsinp(self, fname='', fdir=None):
        ...
        with open(fname, 'r') as rfile:
            ...
            self.grid = radmc3dGrid()
            self.grid.readWavelengthGrid(fdir=fdir)
    ...
    def writeStarsinp(self, ppar=None, wav=None, freq=None, old=False, fdir=None):
    ...
    def getTotalLuminosities(self, readInput=True, fdir=None):
    ...
    def getExternalSource(self, grid=None, ppar=None, ext_source=None):
        """Calculates the irradiation from external source. currently assuming blackbody of a single temperature
        Parameters
        ----------
        ppar : dictionary
               If contains 'ext_source', then the temperature of the external irradiation will be this
        grid : radmc3dGrid()
               needs the wavelength grid
        Returns
        -------
        Boolean
        """
        ...
        if ppar is not None:
            if ext_source is None: # input for ext_source has priority over ext_source in ppar
                if 'ext_source' in ppar:
                    ext_source = ppar['ext_source']
                else:
                    ext_source = 0.

        if ext_source > 0.01:

            self.ext_inten = np.zeros([self.grid.nwav], dtype=np.float64)

            self.ext_inten[:] = 2. * nc.hh * self.grid.freq**3. / nc.cc**2 / (
                                         np.exp(nc.hh * self.grid.freq / nc.kk / ext_source) - 1.0)
        
        return True

    def readExternalSource(self, fname='', binary=False):
        """Reads the intensity of external source
        
        Parameters
        ----------

        fname : str, optional
                Name of the file to read external intensity. If omitted, default 'external_source.inp'
                will be used
        ...
        with open(fname, 'r') as rfile:
            ...
            dum = rfile.readline()
            nlam_this = int(dum)
            if nlam_this != self.grid.nwav:
                raise ValueError('The wavelength grid in wavelength_micron.inp is different '
                                         + 'from that in ' + fname)

            wav = []
            for ilam in range(nlam_this):
                dum = rfile.readline()
                wav.append(float(dum))
                if np.abs((nc.cc / float(dum) * 1e4 - self.grid.wav[ilam] )/ self.grid.wav[ilam]) > 1e-4:
                    raise ValueError('The wavelength grid in wavelength_micron.inp is different '
                                         + 'from that in ' + fname)

            wav_this = np.array(wav, dtype=float)

            inten = []
            for ilam in range(nlam_this):
                dum = rfile.readline()
                inten.append(float(dum))

        inten = np.array(inten, dtype=float)

        self.ext_inten = inten
            
        return True

    def writeExternalSource(self, fname='',  binary=False, fdir=None):
        """Writes the intensity of external source

        Parameters
        ----------

        fname : str, optional
                Name of the file into which the external intensity will be written. If omitted the default
                'external_source.inp' will be used.

        binary : bool, optional
                If True the output will be written in a C-style binary stream, if False the output will be
                formatted ASCII
                This is not in effect yet. will only do ASCII output
        """
        if fname.strip() == '':
            fname = 'external_source.inp'
            if fdir is not None:
        ...
        if self.ext_inten != []:
            ...
            with open(fname, 'w') as wfile:
                # write format number = 2 for now
                wfile.write('%d\n' % 2)

                # write number of elements in wavlength grid
                wfile.write('%d\n' % (self.grid.nwav))
                ...
                # write the intensity as a function of wavelenght
                for ilam in range(self.grid.nwav):
                    wfile.write('%.9e\n' % self.ext_inten[ilam])

        else:
            print('external source intensity is not defined or too low')
            print('will not write external_source.inp')
