...
some bugs/additions I did
- fixed vmin when unable to plot log contours
- added a beck option to plot Beckwith opacity
- added coverplot to overplot contours in plotSlice2D
...
def readData(ddens=False, dtemp=False, gdens=False, gtemp=False, gvel=False, ispec=None, vturb=False, alvec=False, qvis=False, grid=None,
    ...
    qvis  : bool
            If True, heatsource will be read. 0 if no heatsource.inp file
...
def readParams(fdir=None):
    ...
    dum = radmc3dPar()
    dum.readPar(fdir=fdir)
    return dum


def writeDefaultParfile(model='', fdir=None):
...
def getDensVstruct(data=None, vmean_temp=False, ispec_tgas=0, gsize=None, idust=None, 
                   mstar=None, mu=None, kern=None):
    ...
    kern        : ndarray, optional
                  A brute way to obtain smooth temperature profile by convolving 2d with a kernel
    Returns
    -------
    ...
    # improve temperature smoothness by convolving
    if kern is not None:
        kern = np.array(kern)
        from scipy.signal import convolve2d
        for ip in range(dusttemp_dummy.shape[3]):
            dd = dusttemp_dummy[:,:,ip]
            cdd = convolve2d(dd, kern, boundary='symm', mode='same')
            dusttemp_dummy[:,:,ip] = cdd
...
def plotSlice2D(data=None, var='ddens', plane='xy', crd3=0.0, icrd3=None, ispec=-1, xlim=(), ylim=(), log=False,
    ...
    xlog            : bool. Default=False. Not done yet
                      If True, the x axis will be in logarithmic scale. If mirror is set, it will be in log symmetric to x=0

    ylog            : bool. Default=False. Not done yet
                      If True, the y axis will be in logarithmic scale. If y crosses zero, it will be in log symmetric to y=0
    ...
    contours        : bool
                      If True, only contour lines are plotted, if False a colorscale plot will be created

    coverplot       : bool
                      If True, contours will be overplotted to colorscale plot
    ...
    Sph2Cart        : bool, default is False
                      If set this to True, and if the coordinate system use was spherical, 
                      then the plotting will plot this to Cartesian. If it was already in Cartesian, 
    xCart           : array, default is None
                      If given then Sph2Cart will interpolate at these x coordinates
    zCart           : array, default is None
                      If given then Sph2Cart will interpolate at these z coordinates
                      then this parameter will have no effect
    mirror 	    : bool, default is False
                      If set to True, will mirror across radius. Better for near/far side visualizations
    ...
    if Sph2Cart == True and Cart2Sph == True:
        raise ValueError('Sph2Cart and Cart2Sph cannot be true at the same time')

    if data.grid.crd_sys != 'car' and data.grid.crd_sys != 'sph':
        raise ValueError('coordinate system should be car or sph')
...
#        angunit_norm = np.pi / 180.
    ...
    if 'x' in plane:
        ...
        if 'y' in plane:
            ...
            if not octree:
                ...
                grid_x = np.copy(data.grid.x)
                grid_y = np.copy(data.grid.y)
                if lattitude:
                    plot_y = np.pi / 2.0 - data.grid.y

                if icrd3 is None:
                    icrd3 = np.abs(data.grid.z - crd3).argmin()
            if (data.grid.crd_sys == 'car' and Cart2Sph==False) or (data.grid.crd_sys=='sph' and Sph2Cart == True):
                xlabel = r'x ' + linunit_label
                ylabel = r'y ' + linunit_label
                xnorm = linunit_norm
                ynorm = linunit_norm
            if (data.grid.crd_sys =='sph' and Sph2Cart==False) or (data.grid.crd_sys=='car' and Cart2Sph == True):
    ...
    #
    # now regrid the data to the desired coordinates
    # can only do plane=='xy' or 'yx' if spherical
    #
    if data.grid.crd_sys == 'car' and Cart2Sph==True:
        pdb.set_trace()

    if data.grid.crd_sys == 'sph' and Sph2Cart == True and (plane=='xy' or plane=='yx'):
        contCart = crd_trans.contSph2Cart(contSph=pdata, raxis=grid_x, ttaaxis=grid_y,
            xaxis=xCart, zaxis=zCart, 
            log=log, pad=pdata.min())
        pdata = contCart['contCart']
        grid_x = contCart['xaxis']
        grid_y = contCart['zaxis']
        plot_x = grid_x
        plot_y = grid_y
    ...
    if (mirror is True) and (data.grid.crd_sys is 'sph'):
        ...
        plot_x.sort()
        pdata0 = pdata.copy()
        pdata = np.append(np.flip(pdata, axis=0), pdata, axis=0)
    ...
    if not contours:
        ...
        if (Sph2Cart or data.grid.crd_sys == 'car'):
            ax.set_aspect('equal')
    ...
    if contours or coverplot:
        ...
        if cllabel:
            plt.clabel(c, inline=True, 
                           fontsize=cllabel_fontsize, fmt=cllabel_fmt)
...
def plotDustOpac(opac=None, var='kabs', idust=0, beck=-1, ax=None, xlabel=None, ylabel=None, 
    ...
    var             : {'kabs', 'ksca', 'kext', 'g', 'ksca_from_z11'}
    ...
    beck            : int, optional
                      value of beta index for opacity. set to -1 to not plot this
    ...
    wavcut          : numpy array, optional
                      the wavelengths to present the value of the opacity
    ...
    if var.lower().strip() == 'kabs':
        ...
        if ylabel is None:
            ylabel = r'g$_{\rm HG}$'
    elif var.lower().strip() == 'ksca_from_z11':
        if opac.ksca_from_z11[idust].size != x.size:
            msg = 'opac.ksca_from_z11['+("%d" % idust)+'] has a different number of elements than opac.wav['+("%d" % idust)+']'
            raise ValueError(msg)
        y = opac.ksca_from_z11[idust]
        if ylabel is None:
            ylabel = r'$\kappa_{\rm sca_from_z11}$ [cm$&2$/g]'
    ...
    if wavcut is not None:
        nwavcut = wavcut.size
        cutopac = np.array([nwavcut], dtype=np.float64)
        for ii in range(nwavcut):
            f_interp = interpolate.interp1d(x, y, kind='cubic')
            yii = f_interp(wavcut[ii])
            cutopac[ii] = yii
    ...
    if wavcut is not None:
        for ii in range(nwavcut):
            ax.text(wavcut[ii], cutopac[ii], '(%.2e ' % wavcut[ii]+', '+var+' %.2e)' % cutopac[ii])
...
def plotScatmat(opac=None, var='z11', idust=0, iwav=None, wav=None, xvar='ang', iang=None, ang=None, ax=None,
    ...
    var             : {'z11', 'z12', 'z22', 'z33', 'z34', 'z44', 'linpol'}
                      Variable to be plotted
